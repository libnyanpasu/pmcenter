...
namespace pmcenter
{
    internal class CommandRouter
    {
        ...
        private readonly List<IBotCommand> commands = new();
        ...
        public void RegisterCommand(IBotCommand command)
        {
            ...
            {
                throw new ArgumentException($"A command with prefix \"{command.Prefix}\" already exists.",
                    nameof(command));
            }
        ...
        public async Task<bool> Execute(TelegramBotClient botClient, Update update)
        {
            ...
            if (!update.Message.Text.StartsWith(globalPrefix))
            {
                return false;
            }

            IBotCommand command = commands.FirstOrDefault(cmd =>
            {
                ...
                bool prefixMatch = update.Message.Text.TrimStart(globalPrefix)
