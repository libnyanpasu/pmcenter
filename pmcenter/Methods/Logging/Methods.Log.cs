...
namespace pmcenter
{
    public static partial class Methods
    {
        public static partial class Logging
        {
            ...
            {
                if (Vars.CurrentConf?.IgnoredLogModules.Contains(module) == true)
                {
                    return;
                }
                ...
                string file =
                    $"/{Path.GetFileName(Environment.OSVersion.Platform == PlatformID.Unix ? filePath.Replace(@"\", "/") : filePath)}/{callerName}()@L{lineNumber}";
                string output = Vars.CurrentConf?.DisableTimeDisplay != true
