...
namespace pmcenter
{
    internal class CommandLineRouter
    {
        ...
        private readonly List<ICommandLine> commands = new();
        ...
        public void RegisterCommand(ICommandLine command)
        {
            ...
            {
                throw new ArgumentException($"A commandline with prefix \"{command.Prefix}\" already exists.",
                    nameof(command));
            }
        ...
        public async Task<bool> Execute(string cmdLine)
        {
            ...
            foreach (ICommandLine cmdProcess in commands)
            {
